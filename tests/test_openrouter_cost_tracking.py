...
class TestOpenRouterUsageContext(_ProviderIsolationMixin):
    ...
    def test_http_response_capture_uses_actual_openrouter_cost(self):
        import httpx
        ...
        captured = rt._capture_openrouter_usage_from_http_response(response)
        ...
        assert usage.cached_tokens == 20
        assert usage.total_cost_usd == 0.0042
        assert usage.input_cost_usd == 0.0015
        assert usage.output_cost_usd == 0.0027
        assert abs(usage.cache_cost_usd - ((20 * (0.0015 / 120)) * 0.5)) < 1e-12
        assert usage.model_id == "openai/gpt-5.4"
        assert usage.cost_source == "openrouter_api"

    def test_http_response_capture_rejects_non_openrouter_hosts(self):
        import httpx
    ...
    def test_http_response_capture_accepts_alibaba_host_and_keeps_usd_zero(self):
        import httpx
...
class TestRecordCostIntegration(_ProviderIsolationMixin):
    """Tests for _record_cost integration with OpenRouter usage context."""

    def test_record_cost_uses_openrouter_context(self):
        rt = get_runtime()
        accountant = rt.get_cost_accountant()
        accountant._records.clear()
        accountant._by_agent.clear()
        accountant._by_stage.clear()
        rt.clear_openrouter_usage()
        ...
        rt._record_cost(
            stage="test_stage",
            agent_name="test_agent",
            use_openrouter_usage=True,
        )

        summary = accountant.get_summary()
        assert summary["total_cost_usd"] == 0.00123
        assert summary["total_tokens"] == 150
        assert "test-model" in summary["models_used"]
        rt.clear_openrouter_usage()

    def test_record_cost_fallback_to_estimated(self):
        rt = get_runtime()
        accountant = rt.get_cost_accountant()
        accountant._records.clear()
        accountant._by_agent.clear()
    ...
    def test_extract_and_set_usage_from_crew_tracks_alibaba_tokens_only(self):
        rt = get_runtime()
        rt.clear_openrouter_usage()
        crew = SimpleNamespace(
            calculate_usage_metrics=lambda: SimpleNamespace(
                prompt_tokens=75,
                completion_tokens=25,
                total_tokens=100,
        ...
        globals_dict = rt.extract_and_set_usage_from_crew.__globals__
        original_provider = globals_dict.get("ACTIVE_LLM_PROVIDER")
        try:
            globals_dict["ACTIVE_LLM_PROVIDER"] = "alibaba_coding_plan"
            rt.extract_and_set_usage_from_crew(crew, model_id="qwen3.5-plus")
        finally:
            globals_dict["ACTIVE_LLM_PROVIDER"] = original_provider
...
class TestExtractUsageFromCrew(_ProviderIsolationMixin):
    """Tests for extract_and_set_usage_from_crew function."""

    def test_extract_from_mock_crew(self):
        rt = get_runtime()
        rt.clear_openrouter_usage()
        ...
        mock_crew = MockCrew()
        rt.extract_and_set_usage_from_crew(mock_crew, model_id="test-model")
    ...
    def test_extract_from_mock_crew_normalizes_snapshot_model_id(self):
        rt = get_runtime()
        rt.clear_openrouter_usage()
        ...
        rt.extract_and_set_usage_from_crew(
            MockCrew(),
            model_id="z-ai/glm-5-20260211",
        )

        usage = rt.get_last_openrouter_usage()
        assert usage.model_id == "z-ai/glm-5"
        assert usage.cost_source == "crewai_metrics_with_pricing"
        assert abs(
            usage.total_cost_usd
            - (
                (1_000 * (0.72 / 1_000_000))
                + (500 * (2.30 / 1_000_000))
                - (100 * (0.72 / 1_000_000) * 0.5)
            )
        ) < 1e-12

    def test_extract_handles_none_metrics(self):
        rt = get_runtime()
        rt.clear_openrouter_usage()
        ...
        mock_crew = MockCrew()
        rt.extract_and_set_usage_from_crew(mock_crew)
    ...
    def test_extract_skips_zero_tokens(self):
        rt = get_runtime()
        rt.set_openrouter_usage(
            {
                "prompt_tokens": 100,
        ...
        mock_crew = MockCrew()
        rt.extract_and_set_usage_from_crew(mock_crew, model_id="new-model")

        usage = rt.get_last_openrouter_usage()
        assert usage.input_tokens == 100
        assert usage.model_id == "previous-model"

    def test_extract_usage_from_crew_does_not_skip_when_stale_context_is_other_provider(self):
        rt = get_runtime()
        rt.clear_openrouter_usage()
        rt.set_openrouter_usage(
            {
        ...
        try:
            globals_dict["ACTIVE_LLM_PROVIDER"] = "alibaba_coding_plan"
            rt.extract_and_set_usage_from_crew(MockCrew(), model_id="qwen3.5-plus")
        finally:
            globals_dict["ACTIVE_LLM_PROVIDER"] = original_provider
    ...
    def test_record_cost_clear_usage_after_record(self):
        rt = get_runtime()
        accountant = rt.get_cost_accountant()
        accountant._records.clear()
        accountant._by_agent.clear()
...
class TestResilienceModelResolution(_ProviderIsolationMixin):
    ...
    def test_record_cost_clears_usage_by_default(self):
        rt = get_runtime()
        accountant = rt.get_cost_accountant()
        accountant._records.clear()
        accountant._by_agent.clear()
        ...
        usage_after_first = rt.get_last_openrouter_usage()
        assert usage_after_first.total_tokens == 0
