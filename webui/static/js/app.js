...
const SETTINGS_SCHEMA = [
  ...
  { id:'librarian_auth', title:'Librarian Provider Auth', icon:'🔐', open:false,
    keys:['CONTEXT7_API_KEY','GITHUB_TOKEN','TAVILY_API_KEY'] },
  ...
  { id:'run_insights', title:'Run Insights Ledger', icon:'📚', open:false,
    keys:['CRUCIBLE_RUN_INSIGHTS_ENABLED','CRUCIBLE_RUN_INSIGHTS_RECORD_OUTPUT','CRUCIBLE_RUN_INSIGHTS_RECORD_ERRORS',
          ...
          'CRUCIBLE_RUN_INSIGHTS_BACKEND','CRUCIBLE_RUN_INSIGHTS_API_URL','CRUCIBLE_RUN_INSIGHTS_API_TOKEN',
          'CRUCIBLE_RUN_INSIGHTS_API_TIMEOUT_SECONDS','CRUCIBLE_RUN_INSIGHTS_API_MAX_RETRIES',
          'CRUCIBLE_RUN_INSIGHTS_API_BATCH_FLUSH_SECONDS','CRUCIBLE_RUN_INSIGHTS_API_BATCH_SIZE'] },
...
const KEY_META = {
  ...
  CRUCIBLE_RUN_INSIGHTS_BACKEND:          { label:'Storage Backend',       desc:{en:'Storage backend selector. local = JSONL streams under .crucible_insights/ (durable, local-only). dual = local durable copy plus asynchronous sync to a Cloudflare Worker (recommended for cloud). cloudflare = same write/sync path as dual, but reads are served from the cloud Worker. dual/cloudflare require the API URL + token below; if either is missing they degrade safely to local-only.', zh:'儲存後端選擇器。local = .crucible_insights/ 下的 JSONL streams(本機持久化)。dual = 本機持久化副本 + 非同步同步到 Cloudflare Worker(雲端建議用此)。cloudflare = 寫入/同步路徑與 dual 相同,但讀取由雲端 Worker 提供。dual/cloudflare 需要下方的 API URL + token;任一缺少則安全退回 local-only。'}, type:'select', opts:[{v:'local',l:'local (JSONL on disk)'},{v:'dual',l:'dual (local + cloud sync)'},{v:'cloudflare',l:'cloudflare (cloud-primary reads)'}] },
  CRUCIBLE_RUN_INSIGHTS_API_URL:          { label:'Cloud API URL',         desc:{en:'Cloudflare Worker endpoint for the insights ledger, e.g. https://crucible-insights.<sub>.workers.dev. Used only when BACKEND=dual/cloudflare; must be https in production. See cloudflare/insights-worker/ for the deployable Worker.', zh:'Insights 帳本的 Cloudflare Worker 端點,例如 https://crucible-insights.<sub>.workers.dev。僅在 BACKEND=dual/cloudflare 時使用;生產環境必須是 https。可部署的 Worker 見 cloudflare/insights-worker/。'}, type:'text' },
  CRUCIBLE_RUN_INSIGHTS_API_TOKEN:        { label:'Cloud API Token',       desc:{en:'Bearer token sent to the Worker; must match the Worker secret set via `wrangler secret put CRUCIBLE_RUN_INSIGHTS_API_TOKEN`. Stored masked, never committed. Used only when BACKEND=dual/cloudflare.', zh:'送給 Worker 的 Bearer token;必須與 `wrangler secret put CRUCIBLE_RUN_INSIGHTS_API_TOKEN` 設定的 Worker secret 一致。遮罩儲存、絕不 commit。僅在 BACKEND=dual/cloudflare 時使用。'}, type:'password' },
  CRUCIBLE_RUN_INSIGHTS_API_TIMEOUT_SECONDS: { label:'Cloud Timeout (s)',  desc:{en:'Per-request HTTP timeout for cloud-sync POSTs. Cloud sync runs on a background daemon so a hung call never blocks the pipeline, but this bounds each attempt. Range 1-300; default 10.', zh:'雲端同步 POST 的單次 HTTP timeout。雲端同步在背景 daemon 執行,卡住的呼叫不會阻塞主流程,此值限制每次嘗試。範圍 1-300,預設 10。'}, type:'number' },
  CRUCIBLE_RUN_INSIGHTS_API_MAX_RETRIES:  { label:'Cloud Max Retries',     desc:{en:'Retry attempts per batch before deferring to the next flush cycle. Cloud failures only delay sync — local data is never lost (prune refuses to trim un-synced events). Range 0-10; default 3.', zh:'每批次在延到下一輪 flush 前的重試次數。雲端失敗只會延遲同步 — 本機資料絕不遺失(prune 拒絕裁掉未同步事件)。範圍 0-10,預設 3。'}, type:'number' },
  CRUCIBLE_RUN_INSIGHTS_API_BATCH_FLUSH_SECONDS: { label:'Cloud Flush Interval (s)', desc:{en:'How often the background daemon batches un-synced events to the cloud. Lower = fresher cloud copy but more requests; higher = fewer requests. Each write also nudges an immediate flush. Range 1-3600; default 30.', zh:'背景 daemon 多久將未同步事件批次送上雲端。越低 = 雲端副本越新但請求越多;越高 = 請求越少。每次寫入也會推動一次即時 flush。範圍 1-3600,預設 30。'}, type:'number' },
  CRUCIBLE_RUN_INSIGHTS_API_BATCH_SIZE:   { label:'Cloud Batch Size',      desc:{en:'Max events per batch POST. Each oversized-payload event costs one Worker subrequest, so keep this at or below your Cloudflare plan’s subrequest limit (50 free / 1000 paid). Range 1-500; default 100.', zh:'每次批次 POST 的最大事件數。每筆超大 payload 事件耗用一個 Worker subrequest,故此值應 ≤ 你的 Cloudflare 方案 subrequest 上限(免費 50 / 付費 1000)。範圍 1-500,預設 100。'}, type:'number' },
  ...
  TAVILY_API_KEY: { label:'Tavily API Key', desc:{en:'Optional Tavily Search API key (v1.1.13) from https://app.tavily.com (free tier: 1,000 credits/month). Tavily is an opt-in commercial general-web provider that sits between DuckDuckGo and SearXNG in the general fallback chain. It activates ONLY when this holds a real value AND ``tavily`` is added to LIBRARIAN_EXTRA_PROVIDERS; otherwise it no-ops and default behaviour is unchanged. Leave at the placeholder value to keep it disabled. Note: enabling Tavily sends your search queries to a third-party commercial API.', zh:'選填的 Tavily Search API 金鑰（v1.1.13），至 https://app.tavily.com 申請（免費方案每月 1,000 credits）。Tavily 是 opt-in 的商用通用網路搜尋 provider，在 general fallback chain 中位於 DuckDuckGo 與 SearXNG 之間。只有當這裡填入真實值「且」LIBRARIAN_EXTRA_PROVIDERS 有加 tavily 時才會啟用，否則 no-op、預設行為不變。保留 placeholder 值即維持停用。注意：啟用 Tavily 會把搜尋查詢送到第三方商用 API。'}, type:'password' },
